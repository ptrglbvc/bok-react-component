...
export default function useEpub() {
      ...
      if (containerFile) {
        ...
        if (obfLocation) {
          ...
          try {
            ...
            let parsedOpf = parser.parseFromString(opf as string, "text/xml");
  ...
  function getObf(containterContent: string): string {
    let parser = new DOMParser();
    let containterDOM = parser.parseFromString(containterContent, "text/xml");
      ...
      if (itemType.includes("image")) {
        ...
        if (itemLocation) {
          if (itemFile) {
            ...
            let cleanItemContents = await cleanImages(itemContents, itemType);
            let evenMoreCleanItemContents = formatChapters(cleanItemContents);
